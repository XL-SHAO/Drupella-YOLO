...
class yolo_heatmap:
    def __init__(self, weight, device, method, layer, backward_type, conf_threshold, ratio, show_result, renormalize, task, img_size, show_target_labels, show_target_labels_conf):
    ...
    def __call__(self, img_path, save_path,file_name):
        # remove dir if exist
...
def get_params():
    params = {
        'weight': 'runs/detect/train/yolov13-4-CFCM-HDSA-ISM/weights/best.pt', # 现在只需要指定权重即可,不需要指定cfg
        'device': 'cuda:0',
        'method': 'HiResCAM', # GradCAMPlusPlus, GradCAM, XGradCAM, EigenCAM, HiResCAM, LayerCAM, RandomCAM, EigenGradCAM, KPCA_CAM
        'layer': [10, 12, 14, 16, 18],#[10, 12, 14, 16, 18]
        ...
        'img_size':640, # 图像尺寸
        'show_target_labels': True,  # 是否显示检测框上的标签文字
        'show_target_labels_conf': True,  # 是否显示置信度分数
    }
    return params

# 安装依赖包 pip install grad_cam
import time
def main_director(subdir2=None):
    # 当前工程项目根目录
    object_path = os.getcwd()
    test_source_director='datasets/drupella_dataset/images/test/'
    # 测试图片的文件夹路径
    source_path = object_path +"/"+test_source_director
    # 当前工程项目根目录的一级子目录名称
    subdir1="runs/detect/heatmap_noresult"
    # 计数处理图片张数
    i=1
    for file_name in os.listdir(source_path):
        # 当前工程项目根目录的三级子目录名称
        subdir3="/"+subdir1+"/"+subdir2 #+"/"+ file_name[:-4]
        entire_director = object_path + subdir3
        if(os.path.exists(entire_director)==False):
            # 创建文件夹
            os.makedirs(entire_director)
        # 停留1秒时间创建文件夹
        #time.sleep(1)
        sourceimg=test_source_director+file_name
        print('图片名称：', sourceimg,'保存路径：', subdir3,"生成第 "+str(i)+" 张图片的热力图")
        i=i+1
        model = yolo_heatmap(**get_params())
        model(sourceimg, object_path+subdir3,file_name)

if __name__ == '__main__':
    model = yolo_heatmap(**get_params())
    main_director("yolov13-4-CFCM-HDSA-ISM")