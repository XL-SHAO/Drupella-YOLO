...
__all__ = ['Converse2D', 'ConverseBlock', 'ISM']
...
class ISM(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, scale=1, padding_mode='circular', eps=1e-5, act=False):
        ...
        # 【唯一修改】将硬编码的9.0改为可学习的参数
        self.bias_offset = nn.Parameter(torch.tensor(9.0))  # 可学习的偏移量
        self.bias = nn.Parameter(torch.zeros(1, self.in_channels, 1, 1))
    ...
    def forward(self, x):
        ...
        # 【唯一修改】使用可学习的bias_offset替代硬编码的9.0
        biaseps = torch.sigmoid(self.bias - self.bias_offset) + self.eps
        ...
        if self.padding > 0:
            out = out[
                ..., self.padding * self.scale:-self.padding * self.scale, self.padding * self.scale:-self.padding * self.scale]
