...
class TaskAlignedAssigner(nn.Module):
    ...
    def iou_calculation(self, gt_bboxes, pd_bboxes):
        ...
        """
        #from .metrics import sdiou
        #return sdiou(gt_bboxes, pd_bboxes, xywh=False, SDIoU=True).squeeze(-1).clamp_(0) #SDIoU损失函数。此外，若训练过程中，损失为nan，则将amp设置为False。

        #from .metrics import shape_iou
        #return shape_iou(gt_bboxes, pd_bboxes).squeeze(-1).clamp_(0)

        #from .metrics import bbox_mpdiou
        #return bbox_mpdiou(gt_bboxes, pd_bboxes, xywh=False, MPDIoU=True).squeeze(-1).clamp_(0)
