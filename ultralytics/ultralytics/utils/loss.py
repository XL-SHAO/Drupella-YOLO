...
from ultralytics.data.scripts.googles import isImproveLossIoU # type: ignore
from .metrics import bbox_iou, probiou, bbox_multiple_iou, piou, sdiou, shape_iou, wasserstein_loss, bbox_UIoU, bbox_inner_iou, bbox_mpdiou
isUseOtherLoss = False
...
class BboxLoss(nn.Module):
    ...
    def forward(
        ...
        global isUseOtherLoss
        try:
            isUseOtherLoss = isImproveLossIoU()
        except Exception as e:
            pass
        """Compute IoU and DFL losses for bounding boxes."""
        weight = target_scores.sum(-1)[fg_mask].unsqueeze(-1)
        if (isUseOtherLoss):
            # iou = piou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, PIoU2=True) #piou 不需要修改tal.py
            # iou = sdiou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, SDIoU=True) #sdiou 修改tal.py
            # iou = shape_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False) #shape_iou 不需要修改tal.py
            # iou = bbox_UIoU(pred_bboxes[fg_mask], target_bboxes[fg_mask], epoch=100,xywh=True ,UIoU=True) # piou 不需要修改tal.py，但这里的epoch要和训练的轮数保持一致
            # iou = bbox_inner_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, CIoU=True, ratio=0.7) #inner-iou 不需要修改tal.py
            # iou = bbox_mpdiou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, MPDIoU=True) #MPDIoU，修改tal.py
            iou = bbox_multiple_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, WIoU=True, scale=True) #SIoU、EIoU、WIoU，不需要修改tal.py
