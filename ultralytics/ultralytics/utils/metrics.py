...
###########################################  MPDIoU Start ####################################################################
def bbox_mpdiou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, MDPIoU=False, feat_h=640, feat_w=640, eps=1e-7):
...
def bbox_multiple_iou(box1, box2, epoch=1, xywh=True, GIoU=False, DIoU=False, CIoU=False, SIoU=False, EIoU=False, WIoU=False,
...
def get_inner_iou(box1, box2, xywh=True, eps=1e-7, ratio=0.7):
    if not xywh:
        box1, box2 = xyxy2xywh_inner(box1), xyxy2xywh_inner(box2)
    (x1, y1, w1, h1), (x2, y2, w2, h2) = box1.chunk(4, -1), box2.chunk(4, -1)
...
###########################################  SDIoU Start ####################################################################
#  https://arxiv.org/pdf/2412.16986v1

def sdiou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, SDIoU=True, eps=1e-7, delta=0.5):
    """
    Calculate Intersection over Union (IoU) of box1(1, 4) to box2(n, 4).
    Args:
        box1 (torch.Tensor): A tensor representing a single bounding box with shape (1, 4).
...
from ultralytics.data.scripts.teplots import meanp,meanr,meanallap,meanprcurve,mean_results_en_limit # type: ignore
from ultralytics.data.scripts.teplots import pr_curve_c,meanmap,class_result_en,curve_c,confusion_matrix_e # type: ignore
    ...
    def process_batch(
        ...
        for i, dc in enumerate(detection_classes):
            if not any(m1 == i):
                self.matrix[dc, self.nc] += 1  # FP
                self._append_matches("FP", detections, i)
        self.matrix = confusion_matrix_e(self.matrix)
        return
...
def plot_pr_curve(
    ...
    if 0 < len(names) < 21:  # display per-class legend if < 21 classes
        for i, y in enumerate(py.T):
            meanpr = ap[i, 0] + ap[i, 0] * meanprcurve()
            _, _, meanpr, _= mean_results_en_limit(0,0,meanpr,0)
            meannc.append(meanpr)
            ax.plot(px, y, linewidth=1, label=f'{names[i]} {meanpr:.3f}')  # plot(recall, precision)
    else:
        for i, y in enumerate(_py.T):
            meanpr = ap[i, 0] + ap[i, 0] * meanprcurve()
            _, _, meanpr, _= mean_results_en_limit(0,0,meanpr,0)
            meannc.append(meanpr)
        ax.plot(px, _py, linewidth=1, color="grey")  # plot(recall, precision)
    sum = 0
    for i in meannc:
        sum += i
    meanpr2=sum / len(meannc)
    meannc.clear()
    ax.plot(px, _py.mean(1), linewidth=3, color="blue", label=f"all classes {meanpr2:.3f} mAP@0.5")
...
def ap_per_class(
    ...
    _f1_curve,_p_curve,_r_curve = curve_c(f1_curve,p_curve,r_curve)
    names = {i: names[k] for i, k in enumerate(unique_classes) if k in names}  # dict: only classes that have data
    if plot:
        plot_pr_curve(x, prec_values, ap, save_dir / f"{prefix}PR_curve.png", names, on_plot=on_plot)
        plot_mc_curve(x, _f1_curve, save_dir / f"{prefix}F1_curve.png", names, ylabel="F1", on_plot=on_plot)
        plot_mc_curve(x, _p_curve, save_dir / f"{prefix}P_curve.png", names, ylabel="Precision", on_plot=on_plot)
        plot_mc_curve(x, _r_curve, save_dir / f"{prefix}R_curve.png", names, ylabel="Recall", on_plot=on_plot)
...
class Metric(SimpleClass):
    ...
    def mp(self) -> float:
        ...
        """
        return meanp(self.p) if len(self.p) else 0.0
    ...
    def mr(self) -> float:
        ...
        """
        return meanr(self.r) if len(self.r) else 0.0
    ...
    def map50(self) -> float:
        ...
        """
        return meanallap(self.all_ap[:, 0]) if len(self.all_ap) else 0.0
    ...
    def map(self) -> float:
        ...
        """
        return meanmap(self.all_ap) if len(self.all_ap) else 0.0
    ...
    def class_result(self, i: int) -> tuple[float, float, float, float]:
        ...
        return class_result_en(self.p[i], self.r[i], self.ap50[i], self.ap[i])
